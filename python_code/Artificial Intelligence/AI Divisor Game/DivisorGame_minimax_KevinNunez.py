...
def DG_maxValue(s):
    v = -math.inf
    for a in s:
        temp = DG_minValue(result(s, a))
        if temp > v:
            v = temp
    return v


def DG_maxValue_Action(s, turn):
    if len(s) == 1:
        return s[0]
    if turn == 2:
        v = DG_maxValue(s)
...
def divisor_game(name, turn, factors):
    ...
    while human_win and ai_win:
        if turn == 1:
            ...
            print()
            print("Here are the remaining numbers: " + str(current_factors))
            choice_ai = DG_maxValue_Action(current_factors, 2)
        ...
        if turn == 2:
            print("Here are the remaining numbers: " + str(current_factors))
            choice_ai = DG_maxValue_Action(current_factors, 1)
            print("My pick is " + str(choice_ai))
            print()
            if choice_ai == n:
                ai_win = False
